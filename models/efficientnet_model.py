"""
EfficientNet-Lite0 Model for Semiconductor Defect Classification

Architecture:
- Base: EfficientNetB0 (pretrained on ImageNet)
- Custom classification head with dropout and batch normalization
- Optimized for edge deployment (~3.5MB → ~900KB after quantization)
"""
...
from training.config import (
    ...
    EFFICIENTNET_CONFIG, LABEL_SMOOTHING
)
...
def create_efficientnet_model(
    ...
    # Base model - EfficientNetB0 (includes its own preprocessing)
    ...
    if freeze_base:
        ...
        print(f"EfficientNet: Freezing {freeze_until}/{total_layers} layers ({freeze_percent*100:.0f}%)")
    ...
    # Create model
    model = Model(inputs=inputs, outputs=outputs, name="efficientnet_defect_classifier")
...
def compile_model(model: Model, learning_rate: float = 1e-3) -> Model:
    ...
    # Use simple metrics to debug JSON error
    metrics = ['accuracy']
...
def unfreeze_layers(model: Model, unfreeze_percent: float = 0.20) -> Model:
    """
    ...
    if base_model is None:
        print("Warning: Could not find EfficientNet base model")
        # Try to unfreeze from all layers
    ...
    print(f"EfficientNet: {trainable_count}/{total_layers} layers now trainable")
